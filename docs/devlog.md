...
**Later the same day: a copy pass.** Looking at the pages in a browser for the first time (the status note had said since 07-22 that nobody had) showed the prose carried the usual machine-writing signatures: punchline fragments ("Facts, not opinions.", "That's the whole idea.", "Early, and honest about it."), an anaphoric triplet ("No modesty score. No presets. No opinion..."), contrast flourishes ("the easy build and the wrong one"), mirrored clauses, and roughly eighteen em dashes across four pages. Commit `07c369b` rewrites each as a plain sentence with the meaning unchanged, renames the product-page button from "Affiliate link" to "View at {brand}", and uses plainer wording on the sample notice ("These four items are made up"). The method was a small Node replace script that fails if any needle is not found exactly once, so no sentence could be silently skipped. Verified live on every page: zero old phrases, zero em dashes on About and Privacy, and the two that remain elsewhere are the site title tag. Lesson worth keeping: a reviewer reads the copy as well as the catalogue, and generated-sounding prose is its own credibility tax.

**Later still: the affiliate application became a document, and the site got its own email.** Mohamed asked for best practices on getting affiliate approval with a demo site; rather than leave the answer in chat, it became `docs/affiliate-application.md`: the two-gate model (network first, then per-brand), the reviewer checklist mapped to the live site, paste-ready application answers, a rejection playbook, and a tracker. His call on the brand list was "the 8 most mainstream brands women shop at in Canada", which replaced the brief's US-leaning targets with Aritzia, H&M, Zara, Uniqlo, Simons, Reitmans, Dynamite/Garage and Old Navy. Checking each one's program moved Impact to the front of the application queue (Simons and Reitmans both run there) and demoted Rakuten to second-tier brands. Three of the eight have no joinable program and become plain outbound links, which is inside the wargame-08 viability threshold. Then the last cheap credibility item: `hello@kashfedit.com` now exists via Cloudflare Email Routing, forwarding to the project inbox, with MX, SPF and DKIM resolving publicly, and the Privacy page no longer shows a personal Gmail address. One detour worth recording: the domain lives in a different Cloudflare account from the portfolio site, which the nameserver pairs revealed before any dashboard did.

**Evening: the applications went in.** Impact (publisher, individual, editorial content, CAD, site verified by meta tag), a fresh Awin Canada account (editorial content plus comparison engine, clothing and womenswear sectors), and Sovrn Commerce (Google sign-in, site submitted) were all filed the same evening, with the browser driven from here for every field except passwords, terms boxes, captchas and the final submit, which stayed with Mohamed. FlexOffers refused to submit at all: the Continue button produced a generic error and, per the network log, never sent a request, which points at its invisible reCAPTCHA being blocked by an ad blocker. Left for a retry in an Incognito window. Three of four networks submitted, 118 days after the affiliate blocker first appeared in the status note.

**Next:** wait for the three network decisions, retry FlexOffers, then per-brand program requests with the recovery code, establish what that account is, and submit the affiliate application. Then the eval gate (~$1, never run) and wargame 07 Moves 1–3.
